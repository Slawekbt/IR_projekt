...
{	
		...
		{	
			_delay_us(282);
			// time gap between consecutive demodulated IR signal stamps
			raw_signal[i] = ( (PIND & (1<<PIND2))); // save to raw_signal PIND2 value, divide by 0x04 to get one's and zero's, PIND.2 HIGH state == 0x04
			_delay_us(282.5);
		}
		
		for (i=0; i<120; i++)		  // Send to terminal
		{	
